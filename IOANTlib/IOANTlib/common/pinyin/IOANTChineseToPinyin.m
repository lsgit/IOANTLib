#import "IOANTChineseToPinyin.h"
#import "IOANTpinyin.h"
...
@implementation IOANTChineseToPinyin
...
+ (NSString *) pinyinFromChiniseStringAllUpper:(NSString *)string
{
	...
	return [[[NSString alloc] initWithFormat:@"%@",strValue] autorelease];
}
...
+ (NSString *) pinyinFromChiniseStringCapitalUpper:(NSString *)string
{
    ...
    for (int i=0; i< iLen; i++)
    {
		...
		NSString * strRes = FindLetter( nCode );
        NSString * strres = [NSString stringWithFormat:@"%@%@",[[strRes substringToIndex:1] uppercaseString],[[strRes substringFromIndex:1] lowercaseString]];
		strValue = [strValue stringByAppendingString:strres];
